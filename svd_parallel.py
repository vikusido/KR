...
def calculate_metrics(recommendations, test_data, all_movies, k=10):
    ...
    for user_id, rec_movies in rec_dict.items():
        if user_id in test_user_movies.index and rec_movies:
    ...
    with open('75recommendation_metrics.json', 'w') as f:
...
def generate_recommendation_for_user(user_id, user_factors, movie_factors, user_mapper, movie_inv_mapper, train_data, movies):
    return (user_id, hybrid_recommendations(
        user_id, user_factors, movie_factors, user_mapper, movie_inv_mapper, train_data, movies
    ))
...
def main():
    ...
    print("Generating recommendations in parallel...")
    results = Parallel(n_jobs=-1, backend='threading')(
        delayed(generate_recommendation_for_user)(
    ...
    metrics = calculate_metrics(recommendations, test, all_movies, k=K_FOR_METRICS)

    popular_movies = get_popular_movies(train)
    with open('75cold_start_recommendations.json', 'w') as f:
    ...
    rec_df.to_csv('75recommendations.csv', index=False)
